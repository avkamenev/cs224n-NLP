...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            b_r = tf.get_variable("b_r", shape=(self._state_size), initializer=tf.constant_initializer(0.0))
            b_z = tf.get_variable("b_z", shape=(self._state_size), initializer=tf.constant_initializer(0.0))
            b_o = tf.get_variable("b_o", shape=(self._state_size), initializer=tf.constant_initializer(0.0))

            h_t_1 = state
            x_t = inputs

            z_t = tf.sigmoid(tf.matmul(x_t, W_z) + tf.matmul(h_t_1, U_z) + b_z)
            r_t = tf.sigmoid(tf.matmul(x_t, W_r) + tf.matmul(h_t_1, U_r) + b_r)
            o_t = tf.tanh(tf.matmul(x_t, W_o) + tf.matmul(tf.multiply(r_t, h_t_1), U_o) + b_o)
            h_t = tf.multiply(z_t, h_t_1) + tf.multiply((1-z_t), o_t)
