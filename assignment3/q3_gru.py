...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self):
        ...
        # initial_state = cell.zero_state(tf.shape(self.inputs_placeholder)[0], dtype=tf.float32)
        outputs, state = tf.nn.dynamic_rnn(cell, self.inputs_placeholder, dtype=tf.float32, scope = tf.get_variable_scope())
    ...
    def add_training_op(self, loss):
        ...
        clipped_gradients, self.grad_norm = tf.clip_by_global_norm(gradients, self.config.max_grad_norm)
        if self.config.clip_gradients: gradients = clipped_gradients
