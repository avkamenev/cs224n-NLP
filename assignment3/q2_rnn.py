...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        new_sentence = list(sentence[:max_length])
        new_labels = list(labels[:max_length])
        mask = [True]*len(sentence[:max_length])
        if len(new_sentence)<max_length:
            new_sentence.extend([zero_vector] * (max_length-len(new_sentence)))
            new_labels.extend([zero_label] * (max_length-len(new_labels)))
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.mask_placeholder = tf.placeholder(shape=(None, self.config.max_length), dtype=tf.bool)
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(pretrained_embeddings, self.input_placeholder, partition_strategy='mod', name='embeddings')
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h_t = cell(x[:,time_step,:], h_t_1)
