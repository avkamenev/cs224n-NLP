...
class Config:
    ...
    ### YOUR CODE HERE
    n_window_features = 2*n_word_features*window_size + n_word_features # The total number of features used for each window.
    ### END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for n in range(len(sentence)):
            feature_vector=[]
            if n-window_size<0:
                feature_vector.extend([start]*(window_size-n))
                feature_vector.extend(sentence[:n])
            else:
                feature_vector.extend(sentence[(n-window_size):n])
            feature_vector.extend(sentence[n:(n+window_size+1)])
            if n+window_size>(len(sentence)-1):
                feature_vector.extend([end]*(n+window_size-len(sentence)+1))
            # feature_vector
            # [item for sublist in l for item in sublist]
            windowed_data.append(([item for sublist in feature_vector for item in sublist], labels[n]))
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(tf.int32, shape=(None, self.config.n_classes), name='labels_placeholder')
    ...
    def add_embedding(self):
        ...
        pretrained_embeddings = tf.Variable(self.pretrained_embeddings, name='pretrained_embeddings', dtype=tf.float32)
        embeddings = tf.nn.embedding_lookup(pretrained_embeddings, self.input_placeholder, partition_strategy='mod',
                                            name='embeddings')
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(shape=(self.config.n_window_features*self.config.embed_size, self.config.hidden_size), name='W', initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
        U = tf.get_variable(shape=(self.config.hidden_size, self.config.n_classes), name='U', initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]), name='b1', dtype=tf.float32)
        b2 = tf.Variable(tf.zeros([self.config.n_classes]), name='b2', dtype=tf.float32)
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.labels_placeholder, logits=self.pred)
